...
import { getOrders, replaceDraft } from "../utils/storage.js";

export default function PreviousOrders() {
  const { user } = useAuth();
  const navigate = useNavigate();
  const [orders] = useState(() => getOrders().filter((order) => order.userId === user.id));
          ...
          <article key={order.id} className="card overflow-hidden">
            <div className="grid sm:grid-cols-[180px_1fr]">
              <div className="grid grid-cols-2 sm:grid-cols-1">
                <img src={order.outfit.image} alt={order.outfit.title} className="h-40 w-full object-cover sm:h-full" />
                <img src={order.fabricImage} alt="Fabric" className="h-40 w-full object-cover sm:hidden" />
              </div>
              <div className="p-5">
